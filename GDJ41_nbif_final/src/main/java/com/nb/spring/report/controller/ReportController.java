...
//	}
	...
	@RequestMapping(value="reportItem", method=RequestMethod.POST)
	public ModelAndView insertReport(Report r, String memberNo, ModelAndView mv,
		...
		for(MultipartFile mf: upFile) {
			if(!mf.isEmpty()) {
				...
				try {
					...
					ReportImage file=new ReportImage();
					file.setOriReportImgName(orignalFileName);
					r.getReportImages().add(file);
				}catch(IOException e) {
					e.printStackTrace();
				}
			}
